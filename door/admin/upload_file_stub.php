  ...
  if ($_FILES["file"]["error"] > 0)
	...
	$f = fopen($path,"r");
      	while($array = fgetcsv($f,5000,"\t")) 
		{
			...
			$tick_ORDERDATE=date('Y-m-d H:i:s', (strtotime('1/23/2010 9:47')));
			$tick_EVENTDATE=date('Y-m-d', (strtotime($array[1])));
			$tick_ATTENDLAST=ucfirst(trim($array[2]));
			$tick_ATTENDFIRST=ucfirst(trim($array[3]));
			$tick_SHIPLAST=ucfirst(trim($array[2]));
			$tick_SHIPFIRST=ucfirst(trim($array[3]));
			$tick_SHIPADDR=trim('');
			$tick_SHIPCITY=trim('');
			$tick_SHIPSTATE=trim('');
			$tick_SHIPZIP=trim('');
	...
	echo "<A HREF='user/'>user console</A> ";
